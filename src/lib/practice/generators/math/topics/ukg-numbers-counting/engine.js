...
import {
  comparisonEnoughPairs,
  comparisonObjectAssets,
} from "./comparisonAssets.js";
...
const SKIP_COUNT_GROUP_ASSETS = {
  2: [
    {
      imageUrl:
        "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780590812316-tennis-rocket2s.svg",
      singular: "tennis racket",
      plural: "tennis rackets",
    },
    {
      imageUrl:
        "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780590812785-tennis-rocket2s--1-.svg",
      singular: "tennis racket",
      plural: "tennis rackets",
    },
    {
      imageUrl:
        "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780590892171-cats-two.webp",
      singular: "cat",
      plural: "cats",
    },
    {
      imageUrl:
        "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780591265264-badges-two.webp",
      singular: "badge",
      plural: "badges",
    },
    {
      imageUrl:
        "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780591266250-cherries-two.png",
      singular: "cherry",
      plural: "cherries",
    },
    {
      imageUrl:
        "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780591267050-pair-of-heels.webp",
      singular: "heel",
      plural: "heels",
    },
    {
      imageUrl:
        "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780591268606-pair-of-socks.webp",
      singular: "sock",
      plural: "socks",
    },
    {
      imageUrl:
        "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780591269479-swin-fins-two.webp",
      singular: "swim fin",
      plural: "swim fins",
    },
  ],
...
function mcq(questionText, answer, choices, parts = []) {
  ...
  return {
    ...
    solution: {
      sections: [{ type: "text", content: `The answer is ${answer}.` }],
    },
  };
}
...
function tallySvg(count) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (within === 4) {
      marks.push(
        `<line x1="${x - 75}" y1="105" x2="${x}" y2="35" stroke="#334155" stroke-width="8" stroke-linecap="round"/>`,
      );
    } else {
      marks.push(
        `<line x1="${x}" y1="30" x2="${x}" y2="110" stroke="#334155" stroke-width="8" stroke-linecap="round"/>`,
      );
    }
  }
...
function tenFrameSvg(filled, capacity = 10, groupSize = 0) {
  const cells = Array.from({ length: capacity }, (_, index) => {
    ...
    const fill =
      groupSize > 0 && index < filled
        ? SKIP_COUNT_GROUP_COLORS[
            Math.floor(index / groupSize) % SKIP_COUNT_GROUP_COLORS.length
          ]
        : index < filled
          ? "#60a5fa"
          : "#fff";
...
function comparisonImageGrid(
  ...
  if (!asset?.imageUrl)
    return comparisonDots(count, startX, startY, "#60a5fa", columns, gapX);
  return Array.from({ length: count }, (_, index) => {
...
function comparisonBoxesSvg(
  ...
  const leftItems = comparisonImageGrid(
    leftCount,
    leftAsset,
    52,
    startY,
  ...
  const rightItems = comparisonImageGrid(
    rightCount,
    rightAsset,
    376,
    startY,
...
function additionGroupsSvg(first, second, asset) {
  ...
  const secondItems = additionImageGrid(
    second,
    asset,
    28 + firstWidth + gap,
    38,
...
function additionSingleModelSvg(
  total,
  asset,
  activeCount = total,
  capacity = Math.max(5, total),
) {
...
function subtractionExpressionChoices(
  random,
  difference,
  limit,
...
function subtractionModelSvg(
  minuend,
  takeAway,
  asset,
  capacity = Math.max(5, minuend),
) {
...
function generateUkgAdditionQuestion(skill, random) {
  ...
  const [first, second, total] = pickAddends(
    random,
    upper,
    mode === "addition_complete_make_10" ? 10 : null,
  );
  const asset =
    ADDITION_OBJECT_ASSETS[
      randInt(random, 0, ADDITION_OBJECT_ASSETS.length - 1)
    ];
  ...
  if (
    mode === "addition_complete_sentence" ||
    mode === "addition_complete_make_10"
  ) {
...
function generateUkgSubtractionQuestion(skill, random) {
  ...
  if (mode === "subtraction_make_number") {
    ...
    const [targetMinuend, targetTakeAway] = pickSubtractionFact(
      random,
      upper,
      target,
    );
...
function patternShapeSvg({
  ...
  const shapeMarkup =
    ...
    `<rect x="${offset}" y="${offset}" width="${size}" height="${size}" rx="0" ${common}/>`;
...
function makePatternItem(token) {
  const label =
    token.label ||
    [token.sizeLabel, token.color?.label, token.shape]
      .filter(Boolean)
      .join(" ");
  const id =
    token.id ||
...
function buildPatternCopyQuestion({
  questionText,
  items,
  promptIds,
  answerIds,
  ...
  const targets = answerIds.map((_, index) => ({
    id: `slot_${index + 1}`,
    label: "",
  }));
  const answer = Object.fromEntries(
    targets.map((target, index) => [target.id, answerIds[index]]),
  );
  ...
  return {
    ...
    solution: {
      sections: [
        {
          type: "text",
          content: "Copy the pattern into the empty boxes in the same order.",
        },
      ],
    },
  };
...
function generatePatternQuestion(skill, random) {
  ...
  const [firstShape, secondShape] = shuffle(random, PATTERN_BASE_SHAPES).slice(
    0,
    2,
  );
  ...
  if (mode === "pattern_colour") {
    const items = [
      makePatternItem({
        id: `${firstColor.id}_square`,
        color: firstColor,
        shape: "square",
      }),
      makePatternItem({
        id: `${secondColor.id}_square`,
        color: secondColor,
        shape: "square",
      }),
    ];
    const promptIds = repeatPattern(
  ...
  if (mode === "pattern_growing" || mode === "pattern_growing_next_shape") {
    const items = [
      makePatternItem({
        id: `${firstColor.id}_${firstShape}`,
        color: firstColor,
        shape: firstShape,
      }),
      makePatternItem({
        id: `${secondColor.id}_${secondShape}`,
        color: secondColor,
        shape: secondShape,
      }),
    ];
...
function reuseLkg(skill, config, seed) {
  ...
  return generateLkgQuestion({
    ...config,
    logic_type: lkgSkill,
    variables: { ...(config.variables || {}), seed },
  });
}
...
function generatePictographQuestion(skill, random, mode) {
  const fruitSet =
    PICTOGRAPH_FRUIT_SETS[randInt(random, 0, PICTOGRAPH_FRUIT_SETS.length - 1)];
  ...
  const wrongCounts = correctCounts.map((fruit) => {
    const delta =
      random() < 0.6 ? (random() < 0.5 ? 1 : -1) : random() < 0.5 ? 2 : -2;
    return {
      ...fruit,
      count: Math.max(1, Math.min(fruit.max + 2, fruit.count + delta)),
    };
  });

  const allSame = wrongCounts.every(
    (f, i) => f.count === correctCounts[i].count,
  );
  if (allSame)
    wrongCounts[0] = {
      ...wrongCounts[0],
      count: Math.max(1, wrongCounts[0].count + 1),
    };
  ...
  correctCounts.forEach((fruit) => {
    for (let k = 0; k < fruit.count; k++) {
      let slot;
      do {
        slot = `${randInt(random, 0, COLS - 1)}_${randInt(random, 0, ROWS - 1)}`;
  ...
  const solutionText = correctCounts
  ...
  return {
    ...
    solution: {
      sections: [
        {
          type: "text",
          content: `The correct pictograph shows: ${solutionText}.`,
        },
      ],
    },
  };
...
function vennDiagramSvg(
  ...
  const middleItemsSvg = middleItems
    .map((item, idx) => {
      const offsets = [
        [0, -20],
        [0, 20],
  ...
  const outsideItemsSvg = outsideItems
    .map((item, idx) => {
...
function vennDiagramButtonsSvg(
  leftLabel,
  rightLabel,
  leftImage,
  middleImage,
  ...
  const leftItem = leftImage
    ? `<image href="${leftImage}" x="142" y="102" width="36" height="36" preserveAspectRatio="xMidYMid meet"/>`
    : "";
  const middleItem = middleImage
    ? `<image href="${middleImage}" x="202" y="102" width="36" height="36" preserveAspectRatio="xMidYMid meet"/>`
    : "";
  const rightItem = rightImage
    ? `<image href="${rightImage}" x="262" y="102" width="36" height="36" preserveAspectRatio="xMidYMid meet"/>`
    : "";
  const outsideItem = outsideImage
    ? `<image href="${outsideImage}" x="48" y="162" width="36" height="36" preserveAspectRatio="xMidYMid meet"/>`
    : "";
...
function numberCardsSvg(numbers) {
  ...
  const spacing = 15;
  const totalWidth =
    numbers.length * cardWidth + (numbers.length - 1) * spacing + 40;
...
function generateVennDiagramQuestion(skill, random, mode) {
  ...
  if (mode === "venn_sort") {
    ...
    const otherShapes = shuffle(
      random,
      shapesList.filter((s) => s.name !== chosenShape.name),
    );
    ...
    const otherColors = shuffle(
      random,
      colorsList.filter((c) => c.name !== chosenColor.name),
    );
  ...
  return {
    ...
    solution: {
      sections: [
        {
          type: "text",
          content: `Comparing the values from ${direction === "ascending" ? "smallest to largest" : "largest to smallest"} gives: ${correctAnswerLabel}.`,
        },
      ],
    },
  };
...
function generateProgressionQuestion(skill, random, config = {}) {
  ...
  if (mode === "count") {
    ...
    return mcq(
      ...
      answer,
      choicesNear(random, answer, 1, upper),
      [{ type: "svg", content: stickerSvg(answer) }],
    );
  }

  if (
    mode === "ten_frame_count" ||
    mode === "ten_frame_show" ||
    mode === "represent"
  ) {
  ...
  if (mode === "stickers") {
    ...
    const selectedSticker =
      stickerChoices[randInt(random, 0, stickerChoices.length - 1)];
    const itemLabel =
      targetCount === 1 ? selectedSticker.singular : selectedSticker.plural;
    const questionText = `Put ${targetCount} ${itemLabel} in the picture.`;
    return {
      type: useCatV2Contract ? "categorizationv2" : "interactive_stickers",
      interaction: "interactive_stickers",
      ...(useCatV2Contract
        ? { layoutMode: "sticker_scene", renderer: "html" }
        : {}),
      questionText,
      parts: [
        { type: "text", content: questionText },
      ...
      solution: {
        sections: [
          {
            type: "text",
            content: `Place exactly ${targetCount} ${itemLabel}.`,
          },
        ],
      },
    };
  ...
  if (mode === "shadow_match") {
    ...
    const questionText =
      "Match the stickers to their correct shadow shapes in the picture.";
    return {
      type: "interactive_stickers",
      interaction: "interactive_stickers",
      questionText,
      ...
      solution: {
        sections: [
          {
            type: "text",
            content: "Match each sticker to its correct shadow shape.",
          },
        ],
      },
    };
  ...
  if (
    ["one_more", "count_one_more", "one_less", "count_one_less"].includes(mode)
  ) {
    ...
    const objectChoices = [
      {
        imageUrl: "/images/lkg/apple.png",
        singular: "apple",
        plural: "apples",
      },
      {
        imageUrl: "/images/lkg/flower.png",
        singular: "flower",
        plural: "flowers",
      },
      { imageUrl: "/images/lkg/car.png", singular: "car", plural: "cars" },
    ...
    return mcq(
      questionText,
      answer,
      choicesNear(random, answer, 0, upper),
      visual,
    );
  }

  if (
    ["one_more_less", "count_one_more_less", "before_after_between"].includes(
      mode,
    )
  ) {
  ...
  if (
    ...
    return mcq(
      `What number comes next? ${sequence.join(", ")}, __`,
      answer,
      choicesNear(random, answer, 0, upper),
    );
  }
  ...
  if (mode === "fill_ten_frame") {
    ...
    return mcq(
      "How many more counters fill the ten frame?",
      answer,
      choicesNear(random, answer, 1, 10),
  ...
  if (mode === "number_names") {
    const answer = randInt(random, 0, upper);
    const choices = choicesNear(random, answer, 0, upper).map(
      (number) => NUMBER_NAMES[number],
    );
    return mcq(
      `Choose the name of the number ${answer}.`,
      NUMBER_NAMES[answer],
      choices,
    );
  }
  ...
  if (mode === "tens_ones") {
    ...
    return mcq(
      "Count the tens and ones. What number is shown?",
      answer,
      choicesNear(random, answer, 10, upper),
  ...
  if (mode === "learn_skip_twos" || mode === "skip_twos") {
    ...
    return mcq(
      questionText,
      answer,
      choicesByStepAdaptive(random, answer, upper, step, correctStreak),
      [
        {
          type: "svg",
  ...
  if (mode === "skip_twos_ten_frames") {
    ...
    const answer = randInt(random, 1, Math.floor(upper / step)) * step;
    return mcq(
      `Count the counters by ${NUMBER_NAMES[step] || step}s. How many are shown?`,
      answer,
      choicesByStepAdaptive(random, answer, upper, step, correctStreak),
  ...
  if (mode === "compare_enough") {
    ...
    const available = Math.min(
      upper,
      Math.max(1, needed + randInt(random, -2, 2)),
    );
    const pair =
      comparisonEnoughPairs[
        randInt(random, 0, comparisonEnoughPairs.length - 1)
      ];
    const answer = available >= needed ? "yes" : "no";
    return mcq(
      `Are there enough ${pair.supply.plural} for every ${pair.target.singular}?`,
      answer,
      ["yes", "no"],
      [
        {
          type: "svg",
          content: comparisonBoxesSvg(
            needed,
            available,
            pair.target,
            pair.supply,
  ...
  if (mode === "compare_matching" || mode === "compare_counting") {
    ...
    const [leftAsset, rightAsset] = shuffle(
      random,
      comparisonObjectAssets,
    ).slice(0, 2);
    ...
    return mcq(
      ...
      answer,
      [leftAsset.plural, rightAsset.plural],
      [
        {
          type: "svg",
          content: comparisonBoxesSvg(
  ...
  if (mode === "compare_mixed") {
    ...
    const [firstAsset, secondAsset] = shuffle(
      random,
      comparisonObjectAssets,
    ).slice(0, 2);
    const askMore = random() < 0.5;
    const answer = askMore
      ...
      : green < pink
        ...
        : secondAsset.plural;
    return mcq(
      `Are there ${askMore ? "more" : "fewer"} ${firstAsset.plural} or ${secondAsset.plural}?`,
      answer,
      [firstAsset.plural, secondAsset.plural],
      [
        {
          type: "svg",
          content: mixedComparisonSvg(
            green,
            pink,
            firstAsset,
            secondAsset,
  ...
  if (mode === "compare_fewer_more_same") {
    ...
    const relationWord = ["fewer", "same number of", "more"][
      randInt(random, 0, 2)
    ];
    ...
    return mcq(
      questionText,
      statementIsTrue ? "yes" : "no",
      ["yes", "no"],
      [
        {
          type: "svg",
          content: comparisonBoxesSvg(left, right, leftAsset, rightAsset),
          style: {
            maxWidth: "650px",
            margin: "0 auto",
            justifyContent: "center",
  ...
  if (mode === "compare_two_numbers") {
    ...
    return mcq(
      `Which number is ${askLarger ? "larger" : "smaller"}?`,
      askLarger ? Math.max(first, second) : Math.min(first, second),
      shuffle(random, [first, second]),
    );
  }
  ...
  if (mode === "compare_three_numbers") {
    const numbers = shuffle(
      random,
      Array.from({ length: upper + 1 }, (_, number) => number),
    ).slice(0, 3);
    const askLargest = random() < 0.5;
    return mcq(
      `Which number is ${askLargest ? "largest" : "smallest"}?`,
      askLargest ? Math.max(...numbers) : Math.min(...numbers),
      numbers,
    );
  }
...
const UKG_BUTTON_ASSETS = {
  circle: {
    ...
    yellow:
      "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780656381647-round-buttons-yellow.webp",
    red: "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780656380817-round-buttons-red.webp",
    green:
      "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780656377875-round-buttons-green.webp",
  },
  triangle: {
    ...
    yellow:
      "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780656384207-triangular-buttons-yellow.webp",
    red: "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780656383427-triangular-buttons-red.webp",
    green:
      "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1780656382619-triangular-buttons-green.webp",
  },
};
...
function generateUkgClassifyShapesColorQuestion(skill, random, config) {
  ...
  if (isColorSort) {
    ...
    if (difficulty === "easy") {
      ...
      const shapesRed = shuffle(random, [
        "circle",
        "circle",
        "triangle",
        "triangle",
      ]);
      const shapesGreen = shuffle(random, [
        "circle",
        "circle",
        "triangle",
        "triangle",
    ...
    const shuffledItems = shuffle(random, items).map((item) => ({
      ...item,
      imageWidth: 80,
    }));
    const answer = Object.fromEntries(
      shuffledItems.map((item) => [item.id, item.target]),
    );
        ...
        sections: [
          {
            type: "text",
            content:
              'Put all the red buttons in the "Red buttons" group, and all the green buttons in the "Green buttons" group.',
          },
        ],
      },
    };
    ...
    let items = [];
    if (difficulty === "easy") {
      const chosenColor = shuffle(random, [
        "pink",
        "yellow",
        "red",
        "green",
        ...
        sections: [
          {
            type: "text",
            content:
              'Put all the round buttons in the "Round buttons" group, and all the triangular buttons in the "Triangular buttons" group.',
          },
        ],
      },
    };
...
function generateUkgSortingQuestion(skill, random, config) {
  ...
  if (isColorSort) {
    ...
    const categories = [
      {
        id: `${colorA}_group`,
        label: `${colorA.charAt(0).toUpperCase() + colorA.slice(1)} buttons`,
      },
      {
        id: `${colorB}_group`,
        label: `${colorB.charAt(0).toUpperCase() + colorB.slice(1)} buttons`,
      },
    ];
    ...
    if (difficulty === "easy") {
      ...
      const shapesA = shuffle(random, [
        "circle",
        "circle",
        "triangle",
        "triangle",
      ]);
      const shapesB = shuffle(random, [
        "circle",
        "circle",
        "triangle",
        "triangle",
    ...
    return {
      ...
      solution: {
        sections: [
          {
            type: "text",
            content: `Put all the ${colorA} buttons in the "${colorA.charAt(0).toUpperCase() + colorA.slice(1)} buttons" group, and all the ${colorB} buttons in the "${colorB.charAt(0).toUpperCase() + colorB.slice(1)} buttons" group.`,
          },
        ],
      },
    };
    ...
    return {
      ...
      solution: {
        sections: [
          {
            type: "text",
            content: `Put all the round buttons in the "Round buttons" group, and all the triangular buttons in the "Triangular buttons" group.`,
          },
        ],
      },
    };
...
const snakeSvg = (isLong) => {
  ...
  const spotsHtml = spots
    .map(
      (s) =>
        `<circle cx="${s.cx}" cy="${s.cy}" r="3.5" fill="#f59e0b" stroke="#ffffff" stroke-width="0.5" />`,
    )
    .join("");
    ...
    return {
      ...
      solution: {
        sections: [
          {
            type: "text",
            content: `The ${correctName} is the correct choice.`,
          },
        ],
      },
    };
...
function generateUkgMoneyQuestion(skill, random, config) {
  ...
  if (mode === "money_coin_values") {
    ...
    const targetVal =
      allowedValues[Math.floor(random() * allowedValues.length)];
  ...
  if (mode === "money_equivalent_groups") {
    ...
    const generateDistinctGroup = (val, excludeList = []) => {
      const excludeStrings = excludeList.map((arr) =>
        arr.slice().sort().join(","),
      );
      let attempts = 0;
      while (attempts < 30) {
    ...
    const decoy1Coins = generateDistinctGroup(
      targetVal + (random() > 0.5 ? 1 : -1),
      [targetCoins, correctCoins],
    );
    const decoy2Coins = generateDistinctGroup(
      targetVal + (random() > 0.5 ? 2 : -2),
      [targetCoins, correctCoins, decoy1Coins],
    );
    ...
    return {
      ...
      solution: {
        sections: [
          {
            type: "text",
            content: `The target group adds up to ₹${targetVal}. The correct option also adds up to ₹${targetVal}.`,
          },
        ],
      },
    };
    ...
    return {
      ...
      solution: {
        sections: [
          {
            type: "text",
            content: `Group A shows ₹${valA} and Group B shows ₹${valB}. The ${isCompareMore ? "larger" : "smaller"} amount is ${isACorrect ? "Group A" : "Group B"}.`,
          },
        ],
      },
    };
...
const drawJarOfMarbles = (colorA, countA, colorB, countB, random) => {
  ...
  const MARBLE_IMAGES = {
    black:
      "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1781142819264-black-marble.webp",
    blue: "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1781142824300-blue-marble.webp",
    green:
      "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1781142833112-green-marble.webp",
    gren: "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1781142833112-green-marble.webp",
    yellow:
      "https://pub-6d655d3564544704a2d99beb0760355e.r2.dev/images/1781142840665-yellow-marble.webp",
  };

  for (let i = 0; i < colors.length && i < positions.length; i++) {
    const pos = positions[i];
    const colorKey = String(colors[i] || "")
      .toLowerCase()
      .trim();
...
export function generateUkgNumbersCountingQuestion(config = {}) {
  ...
  const skill =
    ukgNumbersCountingSkillMap[skillId] ||
    ukgNumbersCountingSkillMap["ukg-count3-learn"];
  ...
  if (skill.params.mode === "shapes_color") {
    ...
    ["sort_color", "sort_shape", "classify_sort"].includes(skill.params.mode)
  ) {
    ...
    reusedModes.includes(skill.params.mode) &&
    skill.params.limit <= 10
  ) {
    ...
    rawQuestion = generateLkgQuestion({
      ...config,
      logic_type: lkgSkill,
      variables: { ...(config.variables || {}), seed },
    });
  ...
  const hasOnlyNumericOptions =
    Array.isArray(rawQuestion.options) &&
    rawQuestion.options.length > 0 &&
    rawQuestion.options.every((option) =>
      /^-?\d+(?:\.\d+)?$/.test(
        String(option?.label ?? option?.value ?? option),
      ),
    );
  const layoutConfig =
    hasOnlyNumericOptions && !rawQuestion.layoutConfig?.variant
